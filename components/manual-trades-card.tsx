...
export function ManualTradesCard({ trades, onAddTrade, onEditTrade, onDeleteTrade }: ManualTradesCardProps) {
  ...
  return (
    <div className="bg-card/60 border border-border/40 rounded-xl flex flex-col shadow-lg max-h-[380px] overflow-hidden">

      {/* Header — matches CardHeader style from performance tab */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-border/30 bg-background/30">
        <h3 className="text-xs font-black text-muted-foreground uppercase tracking-widest">Historical Logs</h3>
        <button
          onClick={onAddTrade}
        ...
        {currentLogs.length === 0 ? (
          ...
          currentLogs.map((trade) => (
            <div
              ...
              className="flex items-center justify-between p-3 rounded-lg bg-background/40 border border-border/30 group hover:border-border/60 transition-all cursor-pointer"
            >
              ...
              <div className="flex items-center gap-3">
                <span className={`text-sm font-black tabular-nums ${trade.rMultiple >= 0 ? "text-emerald-500" : "text-rose-500"}`}>
