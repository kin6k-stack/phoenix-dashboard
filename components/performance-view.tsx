...
// Performance View — per-account tiles with theme-aware charts
// Equity line = account.color (intentional per-account identity)
// Grid / axis / tooltip / drawdown = theme-aware via useChartTheme()

import { useState, useMemo } from "react"
import { TrendingUp, ShieldAlert, ArrowUpRight, ArrowDownRight, Building2, BarChart2 } from "lucide-react"
import { ResponsiveContainer, LineChart, Line, AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ReferenceLine } from "recharts"
import { useTheme } from "@/lib/use-theme"

// ── Types ─────────────────────────────────────────────────────────────────────
interface Trade { id:string; date:string; symbol:string; setup:string; rMultiple:number; direction?:string; notes?:string; accountId?:string }
interface Account { id:string; accountName:string; color:string; broker:string }
interface PerformanceViewProps { userTrades?:Trade[]; botTrades?:Trade[]; accounts?:Account[] }

// ── Per-theme chart palette ────────────────────────────────────────────────────
function useChartTheme() {
  const { theme } = useTheme()
  const T: Record<string,{grid:string;axis:string;tooltip:{bg:string;border:string};ddStroke:string;ddFill:string}> = {
    "black-white": { grid:"rgba(255,255,255,0.06)", axis:"#6b7280", tooltip:{bg:"#0a0a0a",border:"#2a2a2a"}, ddStroke:"#888", ddFill:"rgba(136,136,136,0.08)" },
    "bloomberg":   { grid:"rgba(255,107,0,0.08)",   axis:"#7a5a3a", tooltip:{bg:"#080806",border:"#3a2200"}, ddStroke:"#ff3d3d", ddFill:"rgba(255,61,61,0.08)" },
    "nord":        { grid:"rgba(136,192,208,0.09)",  axis:"#5c7a8a", tooltip:{bg:"#1e2a34",border:"#3a5060"}, ddStroke:"#bf616a", ddFill:"rgba(191,97,106,0.08)" },
    "cyber":       { grid:"rgba(255,0,204,0.06)",    axis:"#7a3a6a", tooltip:{bg:"#0a020c",border:"#4a004a"}, ddStroke:"#ff3d3d", ddFill:"rgba(255,61,61,0.08)" },
    "dark":        { grid:"rgba(52,211,153,0.06)",   axis:"#4b7a65", tooltip:{bg:"#0a130f",border:"#1a3a2a"}, ddStroke:"#f87171", ddFill:"rgba(248,113,113,0.08)" },
    "midnight":    { grid:"rgba(96,165,250,0.07)",   axis:"#4a6a8a", tooltip:{bg:"#070c14",border:"#0f2040"}, ddStroke:"#f87171", ddFill:"rgba(248,113,113,0.08)" },
    "violet":      { grid:"rgba(168,85,247,0.08)",   axis:"#7c6b99", tooltip:{bg:"#0d0b14",border:"#2d1f4e"}, ddStroke:"#f87171", ddFill:"rgba(248,113,113,0.08)" },
    "gold":        { grid:"rgba(245,158,11,0.08)",   axis:"#8a7040", tooltip:{bg:"#0f0e08",border:"#3a2e10"}, ddStroke:"#ef4444", ddFill:"rgba(239,68,68,0.08)" },
  }
...
function buildChartData(trades:Trade[]) {
  ...
  ;[...trades].sort((a,b)=>new Date(a.date).getTime()-new Date(b.date).getTime())
    .forEach((t,i)=>{
      pnl+=Number(t.rMultiple||0); if(pnl>peak) peak=pnl
...
function calcStats(trades:Trade[]) {
  ...
  return {
    ...
    expectancy:trades.length>0?((wins.length/trades.length)*(gp/(wins.length||1))-(losses.length/trades.length)*(gl/(losses.length||1))).toFixed(2):"0.00",
    wins:wins.length, losses:losses.length,
  }
}

// ── Trade dialog ──────────────────────────────────────────────────────────────
function TradeLedgerDialog({trades,account,onClose}:{trades:Trade[];account:Account|null;onClose:()=>void}) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="w-full max-w-lg rounded-2xl border border-border bg-card shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border"
          ...
          <button onClick={onClose} className="text-muted-foreground hover:text-foreground text-xl leading-none transition-colors px-2">×</button>
        </div>
        <div className="max-h-[380px] overflow-y-auto custom-scrollbar">
          {sorted.length===0
            ? <p className="text-center text-sm text-muted-foreground py-10 italic">No trades found.</p>
            : sorted.map(t=>{
                const isBuy=(t.direction??"BUY").toUpperCase()==="BUY", isWin=t.rMultiple>0
                return (
                  <div key={t.id} className="flex items-center justify-between px-5 py-3 border-b border-border/30 hover:bg-white/[0.02] transition-colors">
                    <div className="flex flex-col gap-0.5 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-black text-foreground">{t.symbol}</span>
                        <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${isBuy?"bg-emerald-500/15 text-emerald-400":"bg-rose-500/15 text-rose-400"}`}>{isBuy?"BUY":"SELL"}</span>
                      </div>
...
// ── Account tile — useChartTheme() called here (correct scope) ────────────────
function AccountTile({account,trades,onOpenLedger}:{account:Account;trades:Trade[];onOpenLedger:()=>void}) {
  const stats = useMemo(()=>calcStats(trades),[trades])
  const chartData = useMemo(()=>buildChartData(trades),[trades])
  const lastPnl = chartData[chartData.length-1]?.pnl??0
  const ct = useChartTheme()   // ← called at the right level
      ...
      <div className="p-4 space-y-4">
        ...
        {trades.length>0 ? (
          <div className="space-y-3">
            {/* Equity Curve — line uses account.color (account identity), grid/axis themed */}
            <div className="rounded-xl border border-border/30 bg-background/20 overflow-hidden">
              <div className="flex items-center justify-between px-3 py-2 border-b border-border/20 bg-background/30">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[9px] font-mono font-bold tracking-widest text-muted-foreground uppercase">Equity Curve</span>
                </div>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={chartData}>
                    ...
                    <YAxis stroke={ct.axis} fontSize={9} tickLine={false} width={44}
                      tickFormatter={v=>`$${v.toFixed(0)}`} domain={["dataMin - 5","dataMax + 5"]}/>
                    <Tooltip formatter={(v:number)=>[`$${v>=0?"+":""}${v.toFixed(2)}`,"P&L"]}
                      contentStyle={{backgroundColor:ct.tooltip.bg,borderColor:ct.tooltip.border,color:"#f8fafc",fontSize:11,borderRadius:8}}/>
            ...
            <div className="rounded-xl border border-border/30 bg-background/20 overflow-hidden">
              <div className="flex items-center gap-2 px-3 py-2 border-b border-border/20 bg-background/30">
                ...
                <span className="text-[9px] font-mono font-bold tracking-widest text-muted-foreground uppercase">Drawdown</span>
              </div>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={chartData}>
                    ...
                    <Area type="monotone" dataKey="drawdown"
                      stroke={ct.ddStroke} fill={ct.ddFill} strokeWidth={1.5}/>
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
          ...
          <div className="h-20 flex items-center justify-center">
            <p className="text-[11px] text-muted-foreground italic font-mono text-center">No trades synced yet for this account.</p>
          </div>
        )}
      </div>
    </div>
...
export function PerformanceView({userTrades=[],accounts=[]}:PerformanceViewProps) {
  ...
  const byAccount=useMemo(()=>{
    ...
    for(const t of userTrades){const id=(t as any).accountId||"unassigned";if(!map[id])map[id]=[];map[id].push(t)}
    return map
  },[userTrades])
  ...
  if(accounts.length===0&&userTrades.length===0) return (
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-3 p-3 bg-card/40 rounded-xl border border-border/40">
        ...
        <div>
          ...
          <p className="text-[10px] text-foreground/50 mt-0.5">{accountsToShow.length} account{accountsToShow.length!==1?"s":""} · {userTrades.length} total trades</p>
        </div>
        <div className="ml-auto text-right">
          ...
          <p className={`text-sm font-black font-mono ${userTrades.reduce((s,t)=>s+t.rMultiple,0)>=0?"text-emerald-400":"text-rose-400"}`}>
            {userTrades.reduce((s,t)=>s+t.rMultiple,0)>=0?"+":""}${userTrades.reduce((s,t)=>s+t.rMultiple,0).toFixed(2)}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 sm:gap-6">
        {accountsToShow.map(acc=>(
          <AccountTile key={acc.id} account={acc} trades={byAccount[acc.id]??[]} onOpenLedger={()=>setDialogAccount(acc)}/>
        ))}
      </div>

      {dialogAccount&&<TradeLedgerDialog trades={dialogTrades} account={dialogAccount} onClose={()=>setDialogAccount(null)}/>}
    </div>
  )
}
