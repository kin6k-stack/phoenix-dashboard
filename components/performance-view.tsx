...
export function PerformanceView({ trades = [] }: PerformanceViewProps) {
  ...
  // Helper to calculate safe stats for any list of trades
  ...
  // Grouping Logic: Categorize every trade into 4 buckets
  const getCategorizedTrades = () => {
    ...
    // Manual = anything that isn't the 3 bots above
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between p-4 bg-card/40 backdrop-blur-md rounded-xl border border-border/40 shadow-sm">
        <div className="flex flex-col gap-0.5">
          ...
          <span className="text-[11px] text-foreground font-medium italic">Auto-categorizing all Database Entries</span>
        </div>
        <div className="flex gap-1 bg-background/50 p-1.5 rounded-lg border border-border/50">
          {(["ALL", "BOT", "MANUAL"] as const).map((mode) => (
            <button key={mode} onClick={() => setFilterMode(mode)} className={`px-4 py-2 rounded-md text-[10px] font-black uppercase tracking-widest ${filterMode === mode ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted/60"}`}>
              {mode}
            </button>
          ))}
        </div>
