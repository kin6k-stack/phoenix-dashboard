...
export function PerformanceView({ trades = [] }: PerformanceViewProps) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center p-4 glass-card rounded-xl shadow-sm gap-4">
        ...
        <div className="flex flex-wrap gap-1 bg-background/50 p-1.5 rounded-lg border border-border/50 w-full md:w-auto">
          {[
            ...
            <button 
              ...
              className={`flex-1 md:flex-none px-4 py-2 rounded-md text-[10px] font-black uppercase tracking-widest transition-all ${filterMode === mode.id ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted/60"}`}
            >
              {mode.label}
            </button>
          ))}
              ...
              <Card key={cat.id} className="glass-card shadow-lg relative overflow-hidden transition-all hover:border-primary/30">
                ...
                <CardHeader className="pb-4 border-b border-border/30">
                  <CardTitle className="flex items-center justify-between text-sm font-black uppercase tracking-widest">
                    ...
                    <span className={`text-[10px] font-mono px-2 py-1 rounded bg-${cat.color}-500/10 text-${cat.color}-400 shrink-0`}>{stats.total} Trades</span>
                  </CardTitle>
                </CardHeader>
                <CardContent className="pt-4 md:pt-6 space-y-4">
                  <div className="grid grid-cols-2 gap-3 md:gap-4">
                    <div className="p-3 bg-background/50 rounded-lg border border-border/20">
                      ...
                      <p className={`text-lg md:text-xl font-black ${stats.netPnL >= 0 ? "text-emerald-400" : "text-rose-400"}`}>${stats.netPnL.toFixed(2)}</p>
                    </div>
                    <div className="p-3 bg-background/50 rounded-lg border border-border/20">
