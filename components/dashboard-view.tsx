...
interface ChartTokens {
  primary:string; fillStart:string; fillEnd:string; glow:string; grid:string; axisText:string
  barFill:string; barGlow:string; win:string; loss:string; winGlow:string; lossGlow:string
  stroke1:string; stroke2:string; stroke3:string; hasGradientStroke:boolean
}

const FALLBACK_TOKENS: ChartTokens = {
  primary:"#e5e5e5", fillStart:"rgba(229,229,229,0.25)", fillEnd:"rgba(229,229,229,0.02)",
  glow:"rgba(255,255,255,0.4)", grid:"hsla(0,0%,40%,0.2)", axisText:"hsl(0 0% 60%)",
  barFill:"#a3a3a3", barGlow:"rgba(255,255,255,0.2)",
  win:"#34d399", loss:"#fb7185", winGlow:"rgba(52,211,153,0.4)", lossGlow:"rgba(251,113,133,0.4)",
  stroke1:"", stroke2:"", stroke3:"", hasGradientStroke:false,
}
...
function readTokens():ChartTokens {
  const s1=readVar("--chart-stroke-1"), s2=readVar("--chart-stroke-2"), s3=readVar("--chart-stroke-3")
  return {
    ...
    stroke1:s1, stroke2:s2, stroke3:s3,
...
function ChartTooltip({active,payload,label,valuePrefix="",labelHeader}:any) {
  ...
  return (
    <div className="px-3 py-2 rounded-lg border bg-card/95 border-border backdrop-blur-md shadow-xl" style={{minWidth:140}}>
      {labelHeader&&<div className="text-[8.5px] font-black uppercase tracking-[0.2em] text-muted-foreground mb-1.5">{labelHeader}</div>}
      {label&&<div className="text-[10px] font-mono text-muted-foreground mb-1.5 pb-1.5 border-b border-border/50">{label}</div>}
      <div className="flex items-baseline gap-2">
        <span className={`text-[10px] ${isPositive?"text-emerald-400":"text-rose-400"}`}>{isPositive?"▲":"▼"}</span>
        <span className="text-sm font-black font-mono text-foreground tabular-nums">{valuePrefix}{isPositive?"+":""}{value.toFixed(2)}</span>
      </div>
    </div>
  )
}
...
function DistributionTooltip({active,payload}:any) {
  if(!active||!payload?.length) return null
  const datum=payload[0], name:string=datum.name, value:number=datum.value, isWin=name==="Wins"
  ...
  return (
    <div className="px-3 py-2 rounded-lg border bg-card/95 border-border backdrop-blur-md shadow-xl">
      <div className="text-[8.5px] font-black uppercase tracking-[0.2em] text-muted-foreground mb-1.5">Distribution</div>
      <div className="flex items-center gap-2">
        <div className="w-1.5 h-1.5 rounded-full" style={{background:dotColor}}/>
        <span className="text-[10px] uppercase tracking-widest text-muted-foreground">{name}</span>
      </div>
      <div className="mt-1 text-base font-black font-mono text-foreground tabular-nums">{value}</div>
    </div>
  )
}
...
export function DashboardView({trades=[]}:{trades:any[]}) {
  ...
  useEffect(()=>{
    ...
    return ()=>{ cancelAnimationFrame(id1); cancelAnimationFrame(id2); clearTimeout(tid); window.removeEventListener("phoenix-settings-changed",read) }
  },[theme,invert])

  // ── Per-theme chart style ─────────────────────────────────────────────────
  const chartStyle=(()=>{
    switch(theme) {
      case "black-white": return {
        areaType:"monotone" as const, lineGlow:"", dotGrid:false, stippleGrid:false, volBars:true,
        strokeWidth:2, paddingAngle:2, innerRadius:50, outerRadius:75, fillOverride:null,
      }
      case "dark": return {
        ...
        dotGrid:true, stippleGrid:false, volBars:false,
        strokeWidth:2.5, paddingAngle:6, innerRadius:50, outerRadius:75, fillOverride:null,
      }
      ...
      case "violet": return {
        ...
        dotGrid:false, stippleGrid:false, volBars:false,
        strokeWidth:2.5, paddingAngle:6, innerRadius:50, outerRadius:75, fillOverride:"url(#violetStipple)",
      }
      case "gold": return {
        areaType:"stepAfter" as const, lineGlow:"", dotGrid:false, stippleGrid:false, volBars:false,
        strokeWidth:1.5, paddingAngle:6, innerRadius:50, outerRadius:75, fillOverride:"rgba(245,158,11,0.08)",
      }
      // ── Bloomberg — stepped orange terminal ─────────────────────────────────
      case "bloomberg": return {
        areaType:"stepAfter" as const,
        lineGlow:"drop-shadow(0 0 4px rgba(255,107,0,0.5))",
        dotGrid:false, stippleGrid:false, volBars:false,
        strokeWidth:1.5, paddingAngle:2, innerRadius:50, outerRadius:75, fillOverride:null,
      }
      // ── Nord — smooth arctic frost ──────────────────────────────────────────
      case "nord": return {
        areaType:"monotone" as const,
        lineGlow:"drop-shadow(0 0 3px rgba(136,192,208,0.45))",
        dotGrid:false, stippleGrid:false, volBars:false,
        strokeWidth:2, paddingAngle:6, innerRadius:50, outerRadius:75, fillOverride:null,
      }
      // ── Cyber — neon magenta + dot grid ────────────────────────────────────
      case "cyber": return {
        areaType:"monotone" as const,
        lineGlow:"drop-shadow(0 0 6px rgba(255,0,204,0.7)) drop-shadow(0 0 14px rgba(255,0,204,0.3))",
        dotGrid:true, stippleGrid:false, volBars:false,
        strokeWidth:2.5, paddingAngle:6, innerRadius:50, outerRadius:75, fillOverride:null,
      }
      default: return {
        areaType:"monotone" as const, lineGlow:"", dotGrid:false, stippleGrid:false, volBars:false,
        strokeWidth:2, paddingAngle:6, innerRadius:50, outerRadius:75, fillOverride:null,
      }
    }
  })()
  ...
  const currAvg=calcAvg(currMonthTrades), prevAvg=calcAvg(prevMonthTrades)
  ...
  const weeklyMap=trades.reduce((acc:any,t)=>{
    ...
    acc[week]=(acc[week]||0)+Number(t.rMultiple||0); return acc
  },{})
  ...
  const volumeData=equityData.map((d,i)=>({...d,vol:1+Math.round(Math.abs(Math.sin(i*1.7))*3)}))
  ...
  return (
    <div className="p-3 sm:p-4 md:p-6 lg:p-8 space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-5">
            ...
            <div className="h-[140px] sm:h-[170px] lg:h-[200px] px-2 pt-2 pb-1 relative">
              {chartStyle.dotGrid&&(
                <div className="absolute inset-0 pointer-events-none opacity-[0.18] z-10"
                  style={{backgroundImage:`radial-gradient(circle,${tokens.primary} 1px,transparent 0)`,backgroundSize:"12px 12px"}}/>
              )}
              {chartStyle.stippleGrid&&(
                <svg className="absolute inset-0 w-full h-full pointer-events-none z-10" style={{opacity:0.18}}>
                  <defs><pattern id="stipple" x="0" y="0" width="8" height="8" patternUnits="userSpaceOnUse"><circle cx="2" cy="2" r="1" fill={tokens.primary}/></pattern></defs>
              ...
              <ResponsiveContainer width="100%" height="100%">
                {chartStyle.volBars?(
                  <ComposedChart data={volumeData} margin={{top:5,right:5,left:-25,bottom:0}}>
                    <defs>
                      <linearGradient id="equityAreaGradient" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor={tokens.fillStart}/><stop offset="95%" stopColor={tokens.fillEnd}/>
                      </linearGradient>
                    </defs>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={tokens.grid}/>
                    <XAxis dataKey="date" hide/><YAxis yAxisId="equity" axisLine={false} tickLine={false} tick={{fontSize:9,fill:tokens.axisText,fontWeight:"bold"}} domain={["auto","auto"]}/><YAxis yAxisId="vol" orientation="right" hide domain={[0,(d:number)=>d*6]}/>
                    <Tooltip content={({active,payload,label})=><ChartTooltip active={active} payload={payload} label={label} labelHeader="Equity" valuePrefix="$"/>}/>
                    <Bar yAxisId="vol" dataKey="vol" fill={tokens.primary} opacity={0.35} radius={[2,2,0,0]} maxBarSize={6}/>
                    <Area yAxisId="equity" type="monotone" dataKey="value" stroke={strokeUrl} strokeWidth={2} fill="url(#equityAreaGradient)"/>
                  </ComposedChart>
                ):(
                  <AreaChart data={equityData} margin={{top:5,right:5,left:-25,bottom:0}}>
                    <defs>
                      ...
                      {tokens.hasGradientStroke&&(
                        <linearGradient id="equityStrokeGradient" x1="0" y1="0" x2="1" y2="0">
                          <stop offset="0%" stopColor={tokens.stroke1}/><stop offset="50%" stopColor={tokens.stroke2}/><stop offset="100%" stopColor={tokens.stroke3}/>
                        </linearGradient>
                      )}
                    ...
                    <Tooltip content={({active,payload,label})=><ChartTooltip active={active} payload={payload} label={label} labelHeader="Equity" valuePrefix="$"/>}/>
                    <Area type={chartStyle.areaType} dataKey="value" stroke={strokeUrl} strokeWidth={chartStyle.strokeWidth}
        ...
        <Card className="p-3 sm:p-4 lg:p-5 border-border/40 bg-card/60 shadow-lg backdrop-blur-md flex flex-col">
          ...
          {(()=>{
            ...
            const winPct=wins/total, lossPct=losses/total
            const C=2*Math.PI*54
            const winArc=C*winPct, lossArc=C*lossPct, lossRot=winPct*360

            // ── Per-theme segment colours ───────────────────────────────────
            const [segWin,segLoss,glowColor]=(()=>{
              switch(theme) {
                case "violet":    return ["#a78bfa","#6d28d9","rgba(167,139,250,0.5)"]
                case "dark":      return ["#34d399","#065f46","rgba(52,211,153,0.5)"]
                case "midnight":  return ["#60a5fa","#312e81","rgba(96,165,250,0.5)"]
                case "gold":      return ["#fcd34d","#92400e","rgba(252,211,77,0.4)"]
                case "bloomberg": return ["#ff6b00","#ff3d3d","rgba(255,107,0,0.5)"]
                case "nord":      return ["#88c0d0","#bf616a","rgba(136,192,208,0.45)"]
                case "cyber":     return ["#00ff88","#ff00cc","rgba(0,255,136,0.6)"]
                ...
                <div className="flex-1 flex items-center justify-center min-h-[180px]">
                  <div className="relative" style={{width:170,height:170}}>
                    <svg width="170" height="170" viewBox="0 0 170 170" className="-rotate-90">
                      <circle cx="85" cy="85" r="54" stroke={tokens.loss} strokeWidth="16" fill="none" strokeDasharray="5 3" opacity={0.4}/>
                      <circle cx="85" cy="85" r="54" stroke={tokens.win} strokeWidth="16" fill="none" strokeDasharray={`${winArc} ${C}`} strokeLinecap="round"/>
                    </svg>
              ...
              <div className="flex-1 flex items-center justify-center min-h-[180px]">
                <div className="relative" style={{width:180,height:180}}>
                  <svg width="180" height="180" viewBox="0 0 180 180" className="-rotate-90"
                    ...
                    <circle cx="90" cy="90" r="54" stroke="rgba(255,255,255,0.05)" strokeWidth="18" fill="none"/>
                    <circle cx="90" cy="90" r="54" stroke={segLoss} strokeWidth="18" fill="none"
                      strokeDasharray={`${lossArc} ${C}`} strokeLinecap="round"
                      style={{transform:`rotate(${lossRot}deg)`,transformOrigin:"50% 50%"}} opacity={0.85}/>
                    <circle cx="90" cy="90" r="54" stroke={segWin} strokeWidth="18" fill="none"
                      strokeDasharray={`${winArc} ${C}`} strokeLinecap="round"/>
                  </svg>
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className="text-2xl font-black text-foreground">{trades.length}</span>
                    <span className="text-[9px] text-muted-foreground uppercase tracking-widest font-bold">Signals</span>
          ...
          <div className="flex items-center justify-around pt-2 mt-1 border-t border-border/30">
            <div className="flex items-center gap-1.5">
              <div className="w-2.5 h-2.5 rounded-full" style={{background:
                theme==="violet" ? "#a78bfa" : theme==="dark" ? "#34d399" : theme==="midnight" ? "#60a5fa"
                : theme==="gold" ? "#fcd34d" : theme==="bloomberg" ? "#ff6b00"
                : theme==="nord" ? "#88c0d0" : theme==="cyber" ? "#00ff88" : tokens.win}}/>
            ...
            <div className="flex items-center gap-1.5">
              <div className="w-2.5 h-2.5 rounded-full" style={{background:
                theme==="violet" ? "#6d28d9" : theme==="dark" ? "#065f46" : theme==="midnight" ? "#312e81"
                : theme==="gold" ? "#92400e" : theme==="bloomberg" ? "#ff3d3d"
                : theme==="nord" ? "#bf616a" : theme==="cyber" ? "#ff00cc" : tokens.loss}}/>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4 lg:gap-5">
            ...
            <div className="h-[110px] sm:h-[130px] px-2 pt-2 pb-1">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={weeklyData} margin={{top:0,right:5,left:-25,bottom:0}}>
                  ...
                  <YAxis axisLine={false} tickLine={false} tick={{fontSize:9,fill:tokens.axisText,fontWeight:"bold"}}/>
                  <Tooltip cursor={{fill:tokens.grid,opacity:0.4}}
                    content={({active,payload,label})=><ChartTooltip active={active} payload={payload} label={label} labelHeader="Weekly P&L" valuePrefix="$"/>}/>
                  <Bar dataKey="pnl" fill={tokens.barFill} radius={[4,4,0,0]} style={{filter:`drop-shadow(0 0 4px ${tokens.barGlow})`}}/>
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <Card className="p-3 sm:p-4 lg:p-5 border-border/40 bg-card/60 shadow-lg backdrop-blur-md flex flex-col">
          <h3 className="text-xs sm:text-sm font-black mb-3 sm:mb-4 flex items-center gap-2 uppercase tracking-widest text-foreground">
            <Activity size={14} className="text-amber-400 drop-shadow-[0_0_5px_rgba(251,191,36,0.5)]"/>
            Recent Activity
          </h3>
