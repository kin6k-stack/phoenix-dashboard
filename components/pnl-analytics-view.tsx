...
interface Trade {
  id:string; date:string; openedAt?:string; symbol:string; setup:string
  rMultiple:number; direction?:string; entryPrice?:number; sl?:number
  tp1?:number; lot?:number; outcome?:string; notes?:string
}
...
// ── Sub-components ────────────────────────────────────────────────────────────
function StatTile({icon:Icon,label,value,sub,valueColor="text-foreground"}:{icon:React.ElementType;label:string;value:string;sub?:string;valueColor?:string}) {
  return (
...
function SymbolBreakdownCard({symbol,total,wins,losses,netPnl}:{symbol:string;total:number;wins:number;losses:number;netPnl:number}) {
  ...
  return (
    <div className="p-3 sm:p-4 rounded-lg bg-card/40 border border-border/30">
      <div className="flex items-center justify-between mb-2">
        ...
        <span className={`text-sm font-black tabular-nums ${netPnl>=0?"text-emerald-400":"text-rose-400"}`}>{fmtCurrency(netPnl)}</span>
      </div>
      ...
      <div className="mt-2 h-1.5 rounded-full overflow-hidden bg-rose-500/20">
        <div className="h-full bg-emerald-500" style={{width:`${(wins/Math.max(1,total))*100}%`}}/>
      </div>
    </div>
  )
}
...
export function PnLAnalyticsView({trades=[]}:{trades:Trade[]}) {
  ...
  const stats=useMemo(()=>{
    if(trades.length===0) return {netPnl:0,winRate:0,profitFactor:0,avgWin:0,avgLoss:0,maxDrawdown:0,expectancy:0,riskReward:0,totalTrades:0,wins:0,losses:0,bestDay:0,worstDay:0,grossWin:0,grossLoss:0,sharpe:null as number|null,avgHoldMin:null as number|null}
    ...
    ;[...trades].sort((a,b)=>new Date(a.date).getTime()-new Date(b.date).getTime()).forEach(t=>{
      running+=Number(t.rMultiple||0); if(running>peak) peak=running
      const dd=peak>0?((peak-running)/peak)*100:0; if(dd>maxDD) maxDD=dd
    })
    const dailyMap:Record<string,number>={}
    for(const t of trades){const k=new Date(t.date).toDateString();dailyMap[k]=(dailyMap[k]||0)+Number(t.rMultiple||0)}
    ...
    if(Object.keys(dailyMap).length>=10){
      const vals=Object.values(dailyMap), mean=vals.reduce((a,b)=>a+b,0)/vals.length
    ...
    for(const t of trades){
      ...
      const mins=(new Date(t.date).getTime()-new Date(t.openedAt).getTime())/60000
      if(mins>0&&mins<10080) holdMins.push(mins)
    }
    const avgHoldMin=holdMins.length>0?holdMins.reduce((a,b)=>a+b,0)/holdMins.length:null
    return {netPnl,winRate,profitFactor,avgWin,avgLoss,maxDrawdown:maxDD,expectancy,riskReward,totalTrades:trades.length,wins:wins.length,losses:losses.length,bestDay,worstDay,grossWin,grossLoss,sharpe,avgHoldMin}
  },[trades])
  ...
  const equityData=useMemo(()=>{
    ...
    return sorted.map(t=>{running+=Number(t.rMultiple||0);return{date:new Date(t.date).toLocaleDateString("en-CA"),value:Number(running.toFixed(2))}})
  },[trades])
  ...
  const bySymbol=useMemo(()=>{
    ...
    for(const t of trades){const tag=assetTag(t.symbol);if(!m[tag])m[tag]={total:0,wins:0,losses:0,netPnl:0};m[tag].total+=1;m[tag].netPnl+=Number(t.rMultiple||0);if(t.rMultiple>0)m[tag].wins+=1;else if(t.rMultiple<0)m[tag].losses+=1}
    return Object.entries(m).sort(([,a],[,b])=>b.total-a.total)
  },[trades])

  const byDayOfWeek=useMemo(()=>{
    const arr=Array.from({length:7},()=>({total:0,wins:0,netPnl:0}))
    for(const t of trades){const dow=new Date(t.date).getDay();arr[dow].total+=1;arr[dow].netPnl+=Number(t.rMultiple||0);if(t.rMultiple>0)arr[dow].wins+=1}
    const maxAbs=Math.max(1,...arr.map(d=>Math.abs(d.netPnl)))
    return arr.map((d,i)=>({day:DOW[i],...d,winRate:d.total>0?Math.round((d.wins/d.total)*100):0,barWidth:Math.abs(d.netPnl)/maxAbs*100}))
  },[trades])

  const monthly=useMemo(()=>{
    const m:Record<number,number[]>={}
    for(const t of trades){const d=new Date(t.date),yr=d.getFullYear(),mo=d.getMonth();if(!m[yr])m[yr]=Array(12).fill(0);m[yr][mo]+=Number(t.rMultiple||0)}
    return m
  },[trades])
  ...
  // ── Per-theme equity curve style — all 8 themes ─────────────────────────
  const ac=(()=>{
    switch(theme) {
      case "black-white": return {
        type:"monotone" as const, glow:"", dotGrid:false, stipple:false, volBars:true,
        stroke:"#e5e5e5", fill1:"rgba(229,229,229,0.10)", fill2:"rgba(0,0,0,0)", sw:2,
      }
      case "dark": return {
        type:"monotone" as const,
        glow:"drop-shadow(0 0 4px #34d399) drop-shadow(0 0 12px rgba(52,211,153,0.5))",
        dotGrid:true, stipple:false, volBars:false,
        stroke:"#34d399", fill1:"rgba(52,211,153,0.30)", fill2:"rgba(52,211,153,0.00)", sw:2.5,
      }
      case "midnight": return {
        type:"monotone" as const,
        glow:"drop-shadow(0 0 4px #60a5fa) drop-shadow(0 0 10px rgba(96,165,250,0.5))",
        dotGrid:true, stipple:false, volBars:false,
        stroke:"#60a5fa", fill1:"rgba(96,165,250,0.28)", fill2:"rgba(96,165,250,0.00)", sw:2.5,
      }
      case "violet": return {
        type:"monotone" as const,
        glow:"drop-shadow(0 0 4px #c084fc) drop-shadow(0 0 10px rgba(192,132,252,0.5))",
        dotGrid:false, stipple:false, volBars:false,
        stroke:"#a855f7", fill1:"url(#aVioletStipple)", fill2:"rgba(168,85,247,0.00)", sw:2.5,
      }
      case "gold": return {
        type:"stepAfter" as const, glow:"", dotGrid:false, stipple:false, volBars:false,
        stroke:"#f59e0b", fill1:"rgba(245,158,11,0.08)", fill2:"rgba(245,158,11,0.00)", sw:1.5,
      }
      // ── Bloomberg — stepped orange terminal ──────────────────────────────
      case "bloomberg": return {
        type:"stepAfter" as const,
        glow:"drop-shadow(0 0 4px rgba(255,107,0,0.5))",
        dotGrid:false, stipple:false, volBars:false,
        stroke:"#ff6b00", fill1:"rgba(255,107,0,0.18)", fill2:"rgba(255,107,0,0)", sw:1.5,
      }
      // ── Nord — smooth arctic frost ────────────────────────────────────────
      case "nord": return {
        type:"monotone" as const,
        glow:"drop-shadow(0 0 3px rgba(136,192,208,0.45))",
        dotGrid:false, stipple:false, volBars:false,
        stroke:"#88c0d0", fill1:"rgba(136,192,208,0.20)", fill2:"rgba(136,192,208,0)", sw:2,
      }
      // ── Cyber — neon magenta + dot grid ──────────────────────────────────
      case "cyber": return {
        type:"monotone" as const,
        glow:"drop-shadow(0 0 6px rgba(255,0,204,0.7)) drop-shadow(0 0 14px rgba(255,0,204,0.3))",
        dotGrid:true, stipple:false, volBars:false,
        stroke:"#ff00cc", fill1:"rgba(255,0,204,0.18)", fill2:"rgba(255,0,204,0)", sw:2.5,
      }
      default: return {
        type:"monotone" as const, glow:"", dotGrid:false, stipple:false, volBars:false,
        stroke:"#5fc77a", fill1:"rgba(95,199,122,0.35)", fill2:"rgba(95,199,122,0.00)", sw:2,
      }
    }
  })()
    ...
    <div className="space-y-4 sm:space-y-5">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          <StatTile icon={DollarSign} label="Net P&L" value={fmtCurrency(stats.netPnl)} valueColor={stats.netPnl>=0?"text-emerald-400":"text-rose-400"}/>
          <StatTile icon={Trophy} label="Win Rate" value={`${stats.winRate.toFixed(1)}%`} valueColor={stats.winRate>=50?"text-emerald-400":"text-amber-400"}/>
          <StatTile icon={TrendingUp} label="Profit Factor" value={Number.isFinite(stats.profitFactor)?stats.profitFactor.toFixed(2):"∞"} valueColor={stats.profitFactor>=1?"text-emerald-400":"text-rose-400"}/>
          <StatTile icon={ArrowUpRight} label="Avg Win" value={fmtCurrency(stats.avgWin)} valueColor="text-emerald-400"/>
          <StatTile icon={ArrowDownRight} label="Avg Loss" value={fmtCurrency(-stats.avgLoss)} valueColor="text-rose-400"/>
          <StatTile icon={TrendingDown} label="Max Drawdown" value={fmtPercent(-stats.maxDrawdown)} valueColor="text-rose-400"/>
        </div>
      </div>

      {/* Secondary metrics */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatTile icon={Target} label="Expectancy" value={fmtCurrency(stats.expectancy)} sub="per trade avg" valueColor={stats.expectancy>=0?"text-emerald-400":"text-rose-400"}/>
        <StatTile icon={Refresh} label="Risk-to-Reward" value={stats.riskReward>0?`${stats.riskReward.toFixed(2)} R`:"—"} sub="avg win / avg loss" valueColor="text-emerald-400"/>
        <StatTile icon={Hash} label="Sharpe Ratio"
          value={stats.sharpe===null?(trades.length<10?"Low Sample":"—"):stats.sharpe.toFixed(2)}
          sub={stats.sharpe===null?(trades.length<10?"needs 10+ trading days":"insufficient variance"):stats.sharpe>=2?"excellent":stats.sharpe>=1?"good":"below target"}
          valueColor={stats.sharpe===null?"text-muted-foreground":stats.sharpe>=2?"text-emerald-400":stats.sharpe>=1?"text-yellow-400":"text-rose-400"}/>
        <StatTile icon={Activity} label="Avg Hold Time"
          value={stats.avgHoldMin===null?"N/A":stats.avgHoldMin<60?`${Math.round(stats.avgHoldMin)}m`:stats.avgHoldMin<1440?`${(stats.avgHoldMin/60).toFixed(1)}h`:`${(stats.avgHoldMin/1440).toFixed(1)}d`}
          sub={stats.avgHoldMin===null?"needs openedAt field":stats.avgHoldMin<60?"scalp range":stats.avgHoldMin<480?"intraday":"swing"}
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-8 gap-2">
        ...
        <MicroStat label="Avg / Trade" value={stats.totalTrades>0?fmtCurrency(stats.netPnl/stats.totalTrades):"—"} valueColor={stats.netPnl>=0?"text-emerald-400":"text-rose-400"}/>
      </div>
      ...
      <div className="rounded-xl border border-border/40 bg-card/60 shadow-lg overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-border/30 bg-background/30">
          <div className="flex items-center gap-2">
            <TrendingUp className="h-3.5 w-3.5" style={{color:ac.stroke}}/>
              ...
              <AreaChart data={equityData} margin={{top:5,right:12,left:-10,bottom:0}}>
                <defs>
                  ...
                  <pattern id="aVioletStipple" x="0" y="0" width="9" height="9" patternUnits="userSpaceOnUse">
                    <rect width="9" height="9" fill="rgba(168,85,247,0.18)"/><circle cx="2" cy="2" r="0.9" fill="rgba(192,132,252,0.70)"/>
                  </pattern>
                </defs>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={`${ac.stroke}22`}/>
                <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{fontSize:9,fill:"hsl(var(--muted-foreground))"}}/>
                ...
                <Area type={ac.type} dataKey="value" stroke={ac.stroke} strokeWidth={ac.sw}
                  ...
                  style={ac.glow?{filter:ac.glow}:undefined}/>
              </AreaChart>
            )}
          </ResponsiveContainer>
        </div>
      ...
      {bySymbol.length>0&&(
        <div className="rounded-xl border border-border/40 bg-card/60 shadow-lg p-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {bySymbol.map(([sym,s])=>(
              <SymbolBreakdownCard key={sym} symbol={sym} total={s.total} wins={s.wins} losses={s.losses} netPnl={s.netPnl}/>
            ))}
          </div>
        </div>
      )}
        ...
        <div className="space-y-2">
          {byDayOfWeek.map(d=>(
            <div key={d.day} className="flex items-center gap-3">
              <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground w-10 sm:w-12 flex-shrink-0">{d.day}</span>
              <div className="flex-1 h-5 sm:h-6 bg-background/40 rounded relative overflow-hidden">
                {d.total>0&&<div className={`h-full ${d.netPnl>=0?"bg-emerald-500/40":"bg-rose-500/40"} transition-all`} style={{width:`${d.barWidth}%`}}/>}
              </div>
          ...
          <table className="w-full text-left min-w-[700px]">
            <thead>
              <tr className="border-b border-border/20 text-[9px] uppercase tracking-widest text-muted-foreground">
                ...
                {MONTHS.map(m=><th key={m} className="px-2 py-2.5 font-black text-center">{m}</th>)}
                <th className="px-3 py-2.5 font-black text-center">YTD</th>
              </tr>
            </thead>
            <tbody>
              {monthlyYears.length===0
                ? <tr><td colSpan={14} className="text-center py-6 text-[11px] text-muted-foreground italic">No data yet</td></tr>
                : monthlyYears.map(yr=>{
                    ...
                    return (
                      <tr key={yr} className="border-b border-border/10 hover:bg-white/[0.02] transition-colors">
                        ...
                        {row.map((val,i)=>{
                          ...
                          return (
                            <td key={i} className="px-2 py-3 text-center">
                              <div className={`text-[10px] sm:text-[11px] font-mono font-black tabular-nums ${val>0?"text-emerald-400":val<0?"text-rose-400":"text-muted-foreground/30"} ${isCurrent?"bg-emerald-500/[0.08] rounded px-1 py-0.5":""}`}>
                        ...
                        <td className="px-3 py-3 text-center">
                          <div className={`text-[10px] sm:text-[11px] font-mono font-black tabular-nums ${ytd>0?"text-emerald-400":ytd<0?"text-rose-400":"text-muted-foreground/30"}`}>
