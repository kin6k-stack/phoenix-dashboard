...
export function AddTradeDialog({ open, onOpenChange, onSubmit, initialDate, existingTrade, trades = [] }: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      {/* CARD COHESION INTEGRATION: Matches the performance card telemetry popups exactly */}
      <DialogContent className="bg-card/95 backdrop-blur-2xl border-border/40 shadow-[0_0_50px_rgba(0,0,0,0.8)] sm:max-w-[450px]">
        <DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-2">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <select className="w-full bg-background border border-border/50 rounded-md px-3 py-2 text-sm text-foreground focus:border-primary outline-none transition-colors appearance-none" value={formData.symbol} onChange={(e) => setFormData({...formData, symbol: e.target.value})}>
            ...
            <div className="space-y-1">
              ...
              <select className="w-full bg-background border border-border/50 rounded-md px-3 py-2 text-sm text-foreground focus:border-primary outline-none transition-colors appearance-none" value={formData.direction} onChange={(e) => setFormData({...formData, direction: e.target.value})}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <input type="text" className="w-full bg-background border border-border/50 rounded-md px-3 py-2 text-sm text-foreground focus:border-primary outline-none transition-colors" value={formData.setup} onChange={(e) => setFormData({...formData, setup: e.target.value})} required/>
            </div>
            <div className="space-y-1">
              <label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Net P&L ($)</label>
              <input type="number" step="0.01" className={`w-full bg-background border rounded-md px-3 py-2 text-sm focus:border-primary outline-none transition-colors font-mono font-black ${Number(formData.rMultiple) < 0 ? 'border-rose-500/30 text-rose-400' : 'border-emerald-500/30 text-emerald-400'}`} value={formData.rMultiple} onChange={(e) => setFormData({...formData, rMultiple: parseFloat(e.target.value)})} required/>
            </div>
          </div>

          <div className="space-y-1">
            <label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">TradingView Image URL</label>
            <input type="url" placeholder="https://www.tradingview.com/x/..." className="w-full bg-background border border-border/50 rounded-md px-3 py-2 text-sm text-foreground focus:border-primary outline-none transition-colors font-mono" value={formData.screenshot} onChange={(e) => setFormData({...formData, screenshot: e.target.value})}/>
          </div>

          <div className="space-y-1">
            <label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Context Notes</label>
            <textarea className="w-full bg-background border border-border/50 rounded-md px-3 py-2 text-sm text-foreground focus:border-primary outline-none transition-colors min-h-[60px]" value={formData.notes} onChange={(e) => setFormData({...formData, notes: e.target.value})} placeholder="What was the setup trigger?"/>
          </div>

          <div className="flex justify-end pt-2">
            <button type="submit" className="bg-primary text-primary-foreground font-black uppercase tracking-widest text-[11px] px-6 py-2 rounded-md shadow-[0_0_15px_rgba(16,185,129,0.3)] hover:opacity-90 transition-opacity cursor-pointer">
              Save Execution
            </button>
          </div>
        </form>
          ...
          <div className="max-h-[140px] overflow-y-auto space-y-2 custom-scrollbar pr-2">
            {dayTrades.length === 0 ? (
              ...
              dayTrades.map((t: any, i: number) => {
                ...
                return (
                  <div key={i} className="flex items-center justify-between p-2.5 rounded-lg bg-background border border-border/40 hover:border-border/60 transition-colors">
