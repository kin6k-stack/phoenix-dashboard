...
export function AddTradeDialog({ open, onOpenChange, onSubmit, initialDate, existingTrade, trades = [] }: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      {/* COHESIVE THEME SHIELD: Anchored into full rich deep terminal slate variables */}
      <DialogContent className="bg-[#070b12] border border-white/5 shadow-[0_0_60px_rgba(0,0,0,0.85)] sm:max-w-[450px] text-foreground rounded-xl backdrop-blur-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-3">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <select className="w-full bg-black/40 border border-white/5 rounded-md px-3 py-2 text-xs text-foreground focus:border-primary outline-none cursor-pointer font-medium tracking-wide appearance-none" value={formData.symbol} onChange={(e) => setFormData({...formData, symbol: e.target.value})}>
            ...
            <div className="space-y-1.5">
              ...
              <select className="w-full bg-black/40 border border-white/5 rounded-md px-3 py-2 text-xs text-foreground focus:border-primary outline-none cursor-pointer font-medium tracking-wide appearance-none" value={formData.direction} onChange={(e) => setFormData({...formData, direction: e.target.value})}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <input type="text" className="w-full bg-black/40 border border-white/5 rounded-md px-3 py-2 text-xs text-foreground focus:border-primary outline-none tracking-wide font-medium" value={formData.setup} onChange={(e) => setFormData({...formData, setup: e.target.value})} required/>
            </div>
            <div className="space-y-1.5">
              <label className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/80">Net Performance ($)</label>
              <input type="number" step="0.01" className={`w-full bg-black/40 border rounded-md px-3 py-2 text-xs focus:ring-0 outline-none font-mono font-black tracking-wide ${Number(formData.rMultiple) < 0 ? 'border-rose-500/20 text-rose-400 bg-rose-950/5' : 'border-emerald-500/20 text-emerald-400 bg-emerald-950/5'}`} value={formData.rMultiple} onChange={(e) => setFormData({...formData, rMultiple: parseFloat(e.target.value)})} required/>
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <input type="url" placeholder="https://www.tradingview.com/x/..." className="w-full bg-black/40 border border-white/5 rounded-md px-3 py-2 text-xs text-foreground focus:border-primary outline-none font-mono tracking-wide" value={formData.screenshot} onChange={(e) => setFormData({...formData, screenshot: e.target.value})}/>
          </div>

          <div className="space-y-1.5">
            <label className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/80">Context Validation Notes</label>
            <textarea className="w-full bg-black/40 border border-white/5 rounded-md px-3 py-2 text-xs text-foreground focus:border-primary outline-none min-h-[60px] tracking-wide font-medium" value={formData.notes} onChange={(e) => setFormData({...formData, notes: e.target.value})} placeholder="Input explicit structural trade triggers or verification metrics..."/>
          </div>
        ...
        {/* Intraday List History Sub-Ledger Row */}
