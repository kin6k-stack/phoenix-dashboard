...
// ──────────────────────────────────────────────────────────────────────────────
// Normalize the bot/setup name from Firebase.
// Handles every naming variant from the MQL5 bots, the sync script,
// Gemini's "Manual Execution" rename, and the original "Manual Entry".
// ──────────────────────────────────────────────────────────────────────────────
function normalizeBotName(raw: string | undefined | null): string {
  ...
  // Known engines — normalize to consistent display names
  ...
  // Any generic "manual" variant → unified label
  if (u.includes("MANUAL"))                                        return "Manual Execution"

  // Unknown engine name — pass through as-is
  return raw.trim()
}
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const liveTrades = snapshot.docs.map(doc => {
        ...
        // ── Bot name: handle BOTH field names ──────────────────────────
        // Original webhook uses `bot`, Gemini's webhook used `botName`
        ...
        return {
          ...
          rMultiple: data.profit !== undefined ? Number(data.profit) : (data.netProfit !== undefined ? Number(data.netProfit) : 0), 
    ...
    try {
      if (trade.id) {
        ...
        await addDoc(collection(db, "trades"), {
          ...
          timestamp: new Date(trade.date),
          notes: trade.notes || "Trade added via web dashboard",
          screenshot: trade.screenshot || ""
        });
      }
